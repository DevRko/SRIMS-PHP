...
if (isset($_GET['req']) && empty($_SESSION['issue_wizard'])) {
    $r = $pdo->prepare("SELECT * FROM requisitions WHERE id = ? AND status = 'APPROVED'");
    $r->execute([$_GET['req']]);
    if ($r->fetch()) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'confirm_issue') {
        ...
        foreach ($lineItems as $l) {
            $maxQty = min((int) $l['approved_qty'], (int) $l['current_stock']);
            $issueQty = max(0, min($maxQty, (int) ($_POST['issue_qty'][$l['item_id']] ?? 0)));
            $lines[] = ['item_id' => $l['item_id'], 'ri_id' => $l['id'], 'qty' => $issueQty, 'price' => (float) $l['unit_price'], 'approved_qty' => (int) $l['approved_qty']];
            if ($issueQty < $l['approved_qty']) $isFullyIssued = false;
        }

        $issuedToId = $_POST['issued_to_id'] ?? $req['user_id'];
        ...
        try {
            ...
            $updLine = $pdo->prepare('UPDATE requisition_items SET issued_qty=? WHERE id=?');
            foreach ($lines as $l) { $updLine->execute([$l['qty'], $l['ri_id']]); }
...
$sql = "SELECT r.*, u.name AS user_name, d.name AS department_name, (SELECT COUNT(*) FROM requisition_items WHERE requisition_id=r.id) AS item_count
        FROM requisitions r JOIN users u ON u.id=r.user_id JOIN departments d ON d.id=r.department_id WHERE r.status = 'APPROVED'";
$params = [];
...
$allUsers = $pdo->query('SELECT * FROM users WHERE is_active = 1 ORDER BY name')->fetchAll();
$inventoryUsers = array_values(array_filter($allUsers, fn($u) => in_array($u['role'], ['INVENTORY_MGR', 'ADMIN'], true)));
    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody>
                ...
                <tr><td colspan="7" class="px-4 py-12 text-center text-[14px] text-text-muted">No approved requisitions waiting to be issued</td></tr>
...
foreach ($selectedLines as $l) {
    $maxIssuable = min((int) $l['approved_qty'], (int) $l['current_stock']);
    if ($maxIssuable >= $l['approved_qty']) $fullCount++; elseif ($maxIssuable > 0) $partialCount++;
    ...
    <div class="lg:col-span-3">
        <div class="rounded-card border border-border bg-surface-card p-card-padding">
            ...
            <table class="w-full text-[12px]">
                ...
                <tbody>
                    <?php foreach ($selectedLines as $l): $maxIssuable = min((int) $l['approved_qty'], (int) $l['current_stock']); ?>
                    <tr class="border-b border-border last:border-0">
                        ...
                        <td class="py-2 text-right font-semibold <?= $l['current_stock'] < $l['approved_qty'] ? 'text-red-600' : 'text-green-600' ?>"><?= (int) $l['current_stock'] ?></td>
        ...
        <div class="rounded-card border border-border bg-surface-card p-card-padding">
            ...
            <div class="space-y-3">
                ...
                <div><label class="mb-1 block text-[12px] font-medium text-text-primary">Issued To *</label>
                    <select name="issued_to_id" form="issueForm" class="w-full rounded-button border border-border px-3 py-2 text-[13px] focus:border-brand-primary focus:outline-none">
                        <?php foreach ($allUsers as $u): ?><option value="<?= e($u['id']) ?>" <?= $u['id'] === $selectedReq['user_id'] ? 'selected' : '' ?>><?= e($u['name']) ?></option><?php endforeach; ?>
                    </select>
                </div>
                <div><label class="mb-1 block text-[12px] font-medium text-text-primary">Received By *</label><input type="text" name="received_by" form="issueForm" value="<?= e($selectedReq['user_name']) ?>" class="w-full rounded-button border border-border px-3 py-2 text-[13px] focus:border-brand-primary focus:outline-none"></div>
